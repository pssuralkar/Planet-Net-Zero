      ...
      <div className="absolute inset-0 pointer-events-none">
        {heroSection.floatingIcons.map((icon, index) => {
          ...
          return (
            <IconComponent
              key={index}
              className={`absolute ${icon.position} ${icon.size} ${icon.color} animate-float`}
                  ...
                  >
                    {button.href ? (
                      <Link href={button.href} className="flex items-center">
                        {button.text}
                        {IconComponent && <IconComponent className={button.variant === 'primary' ? 'ml-2 w-5 h-5' : 'mr-2 w-5 h-5'} />}
                      </Link>
                    ) : (
                      <button className="flex items-center">
                        {IconComponent && button.variant !== 'primary' && <IconComponent className="mr-2 w-5 h-5" />}
                        {button.text}
                        {IconComponent && button.variant === 'primary' && <IconComponent className="ml-2 w-5 h-5" />}
                      </button>
                    )}
                  </Button>
                );
              ...
              {heroSection.trustIndicators.map((indicator, index) => (
                <React.Fragment key={index}>
                  <div className="flex items-center">
                    <span className="font-medium text-gray-900 mr-1">{indicator.value}</span>
                    {indicator.label}
                  </div>
